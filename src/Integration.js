...
class Integration extends React.Component
{
	...
	constructor(props) 
	{
	 ...
	 {
	 	console.log("Integration flatRateCallback flatRateAmount: ", flatRateAmount);
	 	if(isOneTimeFee)
	 	{
	...
	render()
	{
		if(this.state.whatRendersNext == "firstFlatRate")
			return <FlatRate isOneTimeFee = {true} questionText = "Do you have a flat rate amount?" updateParentState = {this.flatRateCallback}
			 			feeArray ={this.state.feeArray} whatRendersNext = "supporting"  />

		if(this.state.whatRendersNext == "supporting")
			return <Supporting feeArray = {this.state.feeArray} hourlyRateArray = {this.state.hourlyRateArray}
			 updateParentState = {this.updateStateCB} whatRendersNext = {this.props.integrationType  == "Custom" ? "adhocConsultingFees":"signOnOneTimeFee"  } integrationType = {this.props.integrationType}/>

		//This only happens in Custom Integration type
		if(this.state.whatRendersNext == "adhocConsultingFees")
			return <FlatRate isOneTimeFee = {true} questionText = "Are there ad-hoc consulting fees for supporting in-house work?" updateParentState = 
					{this.flatRateCallback} whatRendersNext = "signOnOneTimeFee" feeArray = {this.state.feeArray} key = {this.state.whatRendersNext}/> //TODO Ask if one time fee

		if(this.state.whatRendersNext == "signOnOneTimeFee")
			return <FlatRate isOneTimeFee = {true} questionText = "Is there a one-time fee for setting-up sign on?" updateParentState = 
					{this.flatRateCallback} whatRendersNext = "signOnReoccuringFee" feeArray = {this.state.feeArray} key = {this.state.whatRendersNext}/> //Different message on vendor

		if(this.state.whatRendersNext == "signOnReoccuringFee")
			return <FlatRate isOneTimeFee = {false} questionText = "Are there re-occurring fees for maintaining sign-on?" updateParentState = 
					{this.flatRateCallback} whatRendersNext = "vendorFeeOneTimeFee" feeArray = {this.state.feeArray} key = {this.state.whatRendersNext}/> //Different Message Vendor

		if(this.state.whatRendersNext == "vendorFeeOneTimeFee")
			return <FlatRate isOneTimeFee = {true} questionText = {(this.props.integrationType == "Vendor" || this.props.integrationType == "Custom") ? "Are there one-time vendor fees to set-up rostering?" : "Is there a one-time fee to set-up rostering?"} 
						updateParentState = {this.flatRateCallback} whatRendersNext = "venderFeeReoccuringFees" feeArray = {this.state.feeArray} key = {this.state.whatRendersNext}/> //Different Message for Con

		if(this.state.whatRendersNext == "venderFeeReoccuringFees")
			return <FlatRate isOneTimeFee = {false} questionText = {(this.props.integrationType == "Vendor" || this.props.integrationType == "Custom") ? "Are there re-occuring vendor maintenance fees for rostering?" : "Are there re-occurring maintenance fees for rostering?"} updateParentState = 
		...
		if(this.state.whatRendersNext == "manualDataEntry")
			return <YesNoQuestion whatRendersNextOnYes = "whoEntersData" whatRendersNextOnNo = "oneSystemToAnother" 
		...
		if(this.state.whatRendersNext == "whoEntersData")
			return <SelectableDaysAndHours updateParentState = {this.updateStateCB} hourlyRateArray = {this.state.hourlyRateArray} feeArray = {this.state.feeArray} 
					whatRendersNext = "oneSystemToAnother" questionText = "Who enters the  data?"/>

		if(this.state.whatRendersNext == "oneSystemToAnother")
			return <YesNoQuestion whatRendersNextOnYes = "managesFiles" whatRendersNextOnNo = "setupSecurityForFileTransferFees" updateParentState = {this.updateStateCB} 
		...
		if(this.state.whatRendersNext == "setupSecurityForFileTransferFees") //TODO On Custom this will render manualMappingCleaningData
			return <FileTransferFees feeArray = {this.state.feeArray} updateParentState = {this.updateStateCB} whatRendersNext = {this.props.integrationType == "Custom" ? "manualMappingCleaningData"
		...
		if(this.state.whatRendersNext == "vendorProvideUserTraining")
			return <YesNoQuestion whatRendersNextOnYes = "vendorChangeSeparateFeeForTraining" whatRendersNextOnNo ="relatedUserTraining" updateParentState = {this.updateStateCB} 
		...
		if(this.state.whatRendersNext == "vendorChangeSeparateFeeForTraining")
			return <FlatRate isOneTimeFee = {true} questionText = "Does vendor charge a separate fee for training?" updateParentState = 
		...
		if(this.state.whatRendersNext == "relatedUserTraining")
			return <RelatedUserTraining updateParentState = {this.updateStateCB} hourlyRateArray = {this.state.hourlyRateArray} 
		...
		if(this.state.whatRendersNext == "additionalOneTimeFees")
			return <FlatRate isOneTimeFee = {true} questionText = "Are there any additional one-time fees to add?" updateParentState = 
		...
		if(this.state.whatRendersNext == "additionalInHouseCosts")
			return <FlatRate isOneTimeFee = {true} questionText = "Are there any additional in-house non-IT costs to add?" updateParentState = 
